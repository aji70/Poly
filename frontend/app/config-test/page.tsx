    ...
    (async () => {
      try {
        ...
        if (!cancelled && res.data) {
          ...
          setData({
            POLYGON_RPC_URL: d.POLYGON_RPC_URL ?? null,
            TYCOON_POLYGON_CONTRACT_ADDRESS: d.TYCOON_POLYGON_CONTRACT_ADDRESS ?? null,
  ...
  return (
    <main className="min-h-screen w-full bg-[#010F10] p-6 text-white max-w-2xl">
      <div className="space-y-4">
        <div>
          <span className="text-gray-400 block text-sm mb-1">POLYGON_RPC_URL</span>
          <code className="block bg-black/30 px-3 py-2 rounded break-all">
            {data?.POLYGON_RPC_URL ?? "(not set)"}
          </code>
        </div>
        <div>
          <span className="text-gray-400 block text-sm mb-1">TYCOON_POLYGON_CONTRACT_ADDRESS</span>
          <code className="block bg-black/30 px-3 py-2 rounded break-all">
            {data?.TYCOON_POLYGON_CONTRACT_ADDRESS ?? "(not set)"}
          </code>
        </div>
        <div>
